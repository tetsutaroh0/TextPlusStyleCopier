...
def get_settings_file_path():
    try:
        return os.path.join(
            ...
            "ApplyTextPlusStyle_settings.json"
        )
    except Exception:
...
def copy_style_keep_text(src_tool, dst_comp, dst_tool):
    ...
    if dst_text_before is not None:
        try:
            ...
            print("StyledText restore error:", e)
            ok = False
...
def run_apply(resolve, target_color, keep_color):
    ...
    try:
        ...
        if not src_tool:
            result["message"] = "No TextPlus tool was found inside the appended source clip."
            return result

        target_items = find_colored_textplus_items(timeline, target_color, exclude_item=ref_timeline_item)
        if not target_items:
            result["message"] = f"No Text+ clips found with clip color '{target_color}'."
            return result
        ...
        result["message"] = f"Done: {copied_count} succeeded / {failed_count} failed."
        return result

    finally:
        if ref_timeline_item:
...
def show_persistent_ui(resolve):
    if bmd is None:
        print("BlackmagicFusion / UIManager is not available.")
        return
    ...
    def on_run(ev):
        ...
        try:
            ...
            set_status("An error occurred. Please check the console output.")
